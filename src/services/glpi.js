...
const MAIL_RESET_URL = String(process.env.MAIL_RESET_URL || '').trim();
...
const CORPORATE_EMAIL_DOMAINS = String(
    process.env.CORPORATE_EMAIL_DOMAINS || process.env.CORPORATE_EMAIL_DOMAIN || ''
)
    .split(',')
...
async function loginGLPI() {
    ...
    try {
        ...
        console.log(' GLPI Conectado com sucesso.');
        return true;
    } catch (error) {
...
function validarEmailCorporativo(email) {
    ...
    if (!CORPORATE_EMAIL_DOMAINS.length) {
        return { ok: false, error: 'Dominios corporativos nao configurados.' };
    }
...
async function salvarUsuarioVIP(numeroReal, nome, id, USUARIOS_VIP) {
    try {
        ...
        if (USUARIOS_VIP) {
            USUARIOS_VIP[numeroSeguro] = { nome: nome, id: id };
